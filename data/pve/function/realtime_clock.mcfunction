...
execute if score pve.realtime_clock pve.features matches 1 store result score hour pve.clock run \
 scoreboard players operation daytime pve.clock /= hour_divisor pve.clock.settings

# Set Hour Offset (6:00 starts at tick 0)
execute if score pve.realtime_clock pve.features matches 1 run \
...
execute if score pve.realtime_clock pve.features matches 1 \
 if score hour pve.clock matches 24.. run \ 
...
execute if score pve.realtime_clock pve.features matches 1 \
 store result score minute pve.clock run \
 scoreboard players operation daytime pve.clock %= hour_divisor pve.clock.settings
...
execute if score pve.realtime_clock pve.features matches 1 \
 as @a if entity @s[nbt={SelectedItem:{id:"minecraft:clock"}}] run tag @s add pve.clock

# HH:MM if Hour < 10 and Minute < 10    e.g. 0:5 -> 00:05
execute if score pve.realtime_clock pve.features matches 1 \
 if score hour pve.clock matches 0..9 run \ 
...
execute if score pve.realtime_clock pve.features matches 1 \
 if score hour pve.clock matches 10..23 run \ 
 execute if score minute pve.clock matches 0..9 run \
 title @a[tag=pve.clock] actionbar [{text:"DAY ", color:"yellow", bold:false},\ 
 {score:{objective:"pve.clock", name:"day"}, color:"yellow", bold:true}, \ 
...
execute if score pve.realtime_clock pve.features matches 1 \
 as @a unless entity @s[nbt={SelectedItem:{id:"minecraft:clock"}}] run tag @s[tag=pve.clock] remove pve.clock
