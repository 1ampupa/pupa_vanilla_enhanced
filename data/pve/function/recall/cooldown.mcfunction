...
# 3 minutes
execute as @a if score @s pve.recall_cooldown matches 3600 run function pve:.sys/notification/notify_player \
 {text:[{text:"[Recall] ",color:"yellow",bold:true},{text:"Recall will ready in 3 minutes.",color:"yellow",bold:false}]}

# 1 minutes
execute as @a if score @s pve.recall_cooldown matches 1200 run function pve:.sys/notification/notify_player \
 {text:[{text:"[Recall] ",color:"yellow",bold:true},{text:"Recall will ready in 1 minute.",color:"yellow",bold:false}]}

# 30 seconds
execute as @a if score @s pve.recall_cooldown matches 600 run function pve:.sys/notification/notify_player \
...
execute as @a if score @s pve.recall_cooldown matches 0 run function pve:.sys/notification/notify_player \
