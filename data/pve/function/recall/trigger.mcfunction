...
# Check recall point
execute unless data storage pve:recall Pos as @a[tag=pve.recalling] run function pve:.sys/notification/warn_player \
...
execute unless data storage pve:recall Pos as @a[tag=pve.recalling] run tag @s remove pve.recalling

# Casting if all condition are passed
execute if score pve.recall pve.features matches 1 if data storage pve:recall Pos run function pve:recall/casting
